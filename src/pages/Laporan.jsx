...
const getNamaUser = () => {
  try {
    ...
    // FIX UTAMA: pakai field `name`
    return u?.name?.trim() || "-";
  } catch (e) {
    console.error("getNamaUser error:", e);
    return "-";
  }
};
...
const labelStatusPembayaran = (v) => {
  if (v === "sudah") return "Sudah Dibayar";
  if (v === "proses") return "Proses Pembayaran";
    ...
    <div style="display:flex; align-items:center; border-bottom:2px solid #000; padding-bottom:10px; margin-bottom:12px;">
      ...
      <div style="flex:1;">
        ...
        <div style="font-size:11px;margin-top:4px;line-height:1.35;">
          ...
          Mulya Asri Blok J8/26, RT 24 RW 009, Kel. Sukamulya,<br/>
...
const buildTableHtml = ({ data, showStatus, mode = "detail" }) => {
      ...
      <tbody>
        ${data.map(r => `
          <tr>
            ...
            <td style="border:1px solid #000;padding:6px;text-align:right;">Rp ${toIDR(r.total_harga)}</td>
            ${showStatus ? `<td style="border:1px solid #000;padding:6px;">${labelStatusPembayaran(r.status_cair)}</td>` : ``}
          </tr>
        `).join("")}
      </tbody>
    </table>
  ...
  const buildFooterHtml = (userName) => {
  ...
  return `
    ...
    ">
      <div style="width:40%; margin-left:auto; text-align:center;">
        ...
        <b style="text-transform:uppercase;">${userName}</b><br/>
        CV. Aim Teknik
      </div>

      <p style="margin-top:20px; font-size:10px; text-align:center; color:#555;">
        Dokumen ini dihasilkan oleh sistem informasi CV. Aim Teknik dan bersifat final.
      </p>
    </div>
  `;
};
  ...
  return (
    <div className="p-4 md:p-6 space-y-5">
      ...
      <div className="bg-white rounded-2xl shadow p-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
        ...
        <div>
          <label className="text-sm text-gray-600">Status BA Opname</label>
          <select
            value={hasBa}
          ...
          >
            <option value="all">Semua</option>
            <option value="ada">Ada BA Opname</option>
            <option value="tidak">Belum Ada BA Opname</option>
          </select>
        </div>
                ...
                <div className="bg-white rounded-xl shadow-lg w-[95%] md:w-[900px] p-6 max-h-[90vh] overflow-y-auto">
                  ...
                  <div className="border rounded-lg overflow-hidden">
                    <table className="min-w-full text-sm">
                      ...
                      <tbody className="divide-y">
                        {rows.map((r) => (
                          <tr key={r.kode_toko} className="hover:bg-gray-50">
